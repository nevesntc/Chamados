...
- `scripts/backup-production.php create` executou `pg_dump` 17.5 contra o PostgreSQL 17.6, salvou um arquivo cifrado AES-256-GCM em `.tools/backups` e verificou seu formato com `pg_restore --list`. O comando `verify` passou de forma independente. O arquivo foi decifrado e restaurado com `pg_restore --exit-on-error` em PostgreSQL 17 temporário e isolado em `127.0.0.1:55432`: 3 usuários, 5 vínculos de responsáveis (incluindo workspaces pessoais) e 3 chamados. O banco temporário e o dump em claro foram removidos após a conferência. Ainda não há cópia externa nem agendamento.
...
- `SESSION_ENCRYPT=true` foi configurado na Vercel; os cabeçalhos web e a diretiva de cache privado têm teste de integração. O deployment do commit `f14b9d9` ficou Ready, com [CI aprovada](https://github.com/nevesntc/Chamados/actions/runs/35802256600). No domínio público, `/entrar` respondeu HTTP 200 com `X-Frame-Options: DENY`, CSP `frame-ancestors 'none'` e HSTS; as três contas de demonstração autenticaram e as respostas autenticadas tiveram `Cache-Control: no-store` e HSTS. O smoke final de cadastro, login/logout, rotas e persistência passou e removeu seus dados temporários.
...
- [CI do commit 74e5cd3 aprovada](https://github.com/nevesntc/Chamados/actions/runs/35775044139): matriz PHP 8.3/8.4 em SQLite/PostgreSQL, navegador desktop/celular e container.
- [CI do commit 6f77175 aprovada](https://github.com/nevesntc/Chamados/actions/runs/35776213384): matriz PHP 8.3/8.4 em SQLite/PostgreSQL, navegador desktop/celular e container, após a correção visual.
- Vercel: projeto `central-de-chamados` ligado ao GitHub. Após autorização específica, `central-de-chamados-neves.vercel.app` foi adicionado como domínio público; GET `/cadastro` retornou 200 sem SSO. O alias antigo foi removido (GET `/cadastro` retornou 404), e o alias automático do projeto ainda redireciona para o SSO. A proteção `all_except_custom_domains` não foi desativada globalmente. APP_URL foi alterado para o novo endereço e o redeploy de `6f77175` ficou Ready.
- Smoke remoto no novo domínio: conta temporária registrada, redirecionamento para o mesmo host, painel/lista/equipe/perfil HTTP 200, chamado automático criado e aberto, logout bloqueou `/workspace`, novo login recuperou o chamado persistido. O script terminou com saída 0 e removeu conta, workspace e chamado temporários em transação. Convite/isolamento foram verificados na suíte local e CI, não nesse smoke remoto.
- Revisão visual publicada no navegador integrado: cadastro em largura móvel abriu normalmente; título do painel escuro agora aparece branco com contraste legível. A correção veio no commit `6f77175`.
