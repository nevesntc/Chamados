...
- Vercel: projeto existente renomeado para `central-de-chamados` (mesmo ID), novo deployment com autenticação pronto; `chamados-smoky-beta.vercel.app` permanece público. Os aliases gerados com o novo nome levam ao SSO da Vercel. A revisão automática rejeitou desativar essa proteção para todas as URLs e rejeitou uma exceção permanente sem autorização específica. APP_URL foi restaurado ao domínio público existente e iniciou-se um redeploy. O smoke completo será registrado após ficar pronto.
- Revisão visual da tela de login/cadastro publicada no navegador integrado revelou título claro com contraste insuficiente no painel escuro em largura móvel. A cor foi corrigida no CSS e aguarda a próxima publicação.
