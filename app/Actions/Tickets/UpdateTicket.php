...
class UpdateTicket
{
    ...
    public function execute(Ticket $ticket, array $data, int $workspaceId): Ticket
    {
        return $this->transaction->run(function () use ($ticket, $data, $workspaceId) {
            $ticket->update([
                'title' => $data['title'], 'description' => $data['description'],
                'priority' => $data['priority'], 'status' => $data['status'],
                'assignee_id' => $data['assignment_mode'] === 'automatic'
                    ? $this->selector->select($workspaceId, $ticket->id) : $data['assignee_id'],
            ]);
