...
- A sessão dura 30 minutos de inatividade e termina ao fechar o navegador, sem cookie persistente em disco. É um sistema interno, muitas vezes aberto em máquina compartilhada, então preferi exigir login de novo a deixar uma sessão longa esquecida. O logout invalida a sessão no servidor e as páginas autenticadas respondem `no-store`, então voltar pelo histórico não mostra conteúdo de quem saiu.
